...
public class AltBn128 {
    ...
    // public static byte[] myPair(byte[] g1_point, byte[] g2_point) throws Exception {
    public static byte[] myPair(byte[] g1_point, byte[] g2_point) throws Exception {
        // assert valid data.
        // todo: convert assert to runtime assertion in AVM
        assert (g1_point != null && g1_point != null &&
                g1_point.length == G1_POINT_SIZE && g2_point.length == G2_POINT_SIZE); // data is well-aligned
        byte[] result = Holder.INSTANCE.myPair(g1_point, g2_point);
        return result;
    }

    public static byte[] gtPow(byte[] gt_point, BigInteger exp) throws Exception {

        assert (exp != null && exp.signum() != -1); // scalar can't be negative (it can be zero or positive)

        byte[] edata = exp.toByteArray();
        assert (edata.length <= WORD_SIZE);

        byte[] edata_aligned = new byte[WORD_SIZE];
        System.arraycopy(edata, 0, edata_aligned, WORD_SIZE - edata.length, edata.length);

        // call jni
        byte[] result = Holder.INSTANCE.gtPow(gt_point, edata_aligned);
        return result;
    }
