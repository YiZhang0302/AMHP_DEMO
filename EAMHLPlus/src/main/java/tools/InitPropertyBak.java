...
public class InitPropertyBak {
    ...
    public void initServer() {
        ...
        String serverRsaPk = RSAEncryption.getPublicKeyString(serverRsa.getPublicKey());
        String serverRsaSk = RSAEncryption.getPrivateKeyString(serverRsa.getPrivateKey());
        ...
        try {
            InputStream in = new BufferedInputStream(new FileInputStream(fileName));
            prop.load(in);
            prop.setProperty("SERVER_RSA_PUBLIC_KEY", serverRsaPk);
            prop.setProperty("SERVER_RSA_PRIVATE_KEY", serverRsaSk);
    ...
    public void initClient() {
        ...
        try {
            ...
            prop.put("CLIENT_RSA_PUBLIC_KEY", clientRsaPk);
            prop.put("NODE_" + port + "_RSA_PUBLIC_KEY", clientRsaPk);
            prop.put("CLIENT_RSA_SECRET_KEY", clientRsaSk);
            prop.put("NODE_" + port + "_RSA_PRIVATE_KEY", clientRsaSk);
    ...
    public static void main(String[] args) throws IOException, NoSuchProviderException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, CipherException {
        InitPropertyBak initProperty = new InitPropertyBak();
